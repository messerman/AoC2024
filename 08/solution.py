...
PARTS = [1, 2]
FILES = ['sample1.txt', 'sample2.txt', 'sample3.txt', 'sample.txt', 'input.txt']
...
def solution1(my_input: list[str]) -> int:
    grid: Grid = parse(my_input)

    antennas: dict[str, list[GridCell]] = {}
    for antenna in filter(lambda cell: cell.value != '.' , grid.cells.values()):
    ...
    for frequency in frequencies:
        for i in range(len(antennas[frequency]) - 1):
            for j in range(i+1 ,len(antennas[frequency])):
                ...
                l = 2 * a1 - a2
                r = 2 * a2 - a1
                if 2*(l - a1) == (l - a2) and grid.in_bounds(l.to_tuple()):
                    antinodes.add(l.to_tuple())
                if 2*(r - a2) == (r - a1) and grid.in_bounds(r.to_tuple()):
    ...
    for frequency in frequencies:
        for i in range(len(antennas[frequency]) - 1):
            for j in range(i+1 ,len(antennas[frequency])):
                ...
                distance = a1 - a2
                tmp = a1
                while grid.in_bounds(tmp.to_tuple()):
                    antinodes.add(tmp.to_tuple())
                    tmp += distance
                tmp = a2
                while grid.in_bounds(tmp.to_tuple()):
                    antinodes.add(tmp.to_tuple())
